...
#include <FlockDetection.h>  // Flock Safety / Raven / Penguin surveillance device detection
...
// Flock Safety surveillance device detection
FlockDetector flockDetector;
unsigned int flock_detection_count = 0;
File flock_json_file;
...
String ota_hostname = "kourosh-forti-hands.github.io";
    ...
    for(int scan_channel = 1; scan_channel < 12; scan_channel++){
      ...
      if (n > 0){
        ...
        for (int i = 0; i < n; i++) {
          ...
          // ========== FLOCK SAFETY SURVEILLANCE DETECTION ==========
          // Check if this device matches Flock Safety / Raven / Penguin patterns
          GPSSnapshot gps_snapshot = get_gps_snapshot();
          FlockDetection* detection = flockDetector.classify(
            this_bssid,
            ssid.c_str(),
            WiFi.RSSI(i),
            gps_snapshot,
          ...
          if (detection != nullptr) {
            ...
            flock_detection_count++;

            // Write to Flock-specific JSON log
            if (!flock_json_file) {
              String flock_filename = "/flock_detections_" + String(bootcount) + ".json";
              flock_json_file = SD.open(flock_filename.c_str(), FILE_APPEND);
            }

            if (flock_json_file) {
              String json_output = flockDetector.toJSON(detection);
              flock_json_file.println(json_output);
              flock_json_file.flush();
            }

            // Alert on LCD (if available)
            Serial.println("!!! FLOCK DEVICE DETECTED !!!");
            Serial.print("Type: ");
            Serial.println(detection->device_type);
            Serial.print("Threat Score: ");
            Serial.println(detection->threat_score);
            Serial.print("GPS: ");
            Serial.print(detection->gps.latitude, 6);
            Serial.print(", ");
            Serial.println(detection->gps.longitude, 6);
          }
          // ========== END FLOCK DETECTION ==========
...
String parse_bside_line(String buff){
  ...
  */
  ...
  // ========== RAVEN GUNSHOT DETECTOR BLE DETECTION ==========
  if (buff.indexOf("BL_RAVEN,") > -1) {
    // Enhanced BLE format from Module B: BL_RAVEN,RSSI,MAC,NAME,UUIDs
    int startpos = buff.indexOf("BL_RAVEN,")+9;
    int endpos = buff.indexOf(",",startpos);
    String rssi = buff.substring(startpos,endpos);
    ...
    startpos = endpos+1;
    endpos = buff.indexOf(",",startpos);
    String ble_name = buff.substring(startpos,endpos);

    startpos = endpos+1;
    String service_uuids = buff.substring(startpos);
    service_uuids.trim();
    ...
    if (6 == sscanf(mac_str.c_str(), "%x:%x:%x:%x:%x:%x%*c", &values[0], &values[1], &values[2], &values[3], &values[4], &values[5])){
      ...
      if (!seen_mac(mac_bytes)){
        ...
        // RAVEN GUNSHOT DETECTOR DETECTED!
        flock_detection_count++;
        Serial.println("!!! RAVEN GUNSHOT DETECTOR (BLE) !!!");
        Serial.print("MAC: ");
        Serial.println(mac_str);
        Serial.print("UUIDs: ");
        Serial.println(service_uuids);

        // Create FlockDetection record with GPS binding
        GPSSnapshot gps_snapshot = get_gps_snapshot();
        FlockDetection* detection = flockDetector.classifyBLE(
          mac_str.c_str(),
          ble_name.c_str(),
          service_uuids.c_str(),
          rssi.toInt(),
        ...
        // Also write to standard Wigle CSV for compatibility
        mac_str.toUpperCase();
        out = mac_str + "," + ble_name + ",[BLE-RAVEN]," + dt_string(get_epoch()) + ",0," + rssi + "," + gps_string() + ",BLE";
      }
    }
  }
  // ========== END RAVEN BLE DETECTION ==========
...
/**
 * Create GPSSnapshot from current NMEA GPS data
 * Used for binding GPS coordinates to Flock detections
 */
GPSSnapshot get_gps_snapshot() {
  GPSSnapshot gps;

  // Get coordinates (NMEA returns microdegrees)
  gps.latitude = (double)nmea.getLatitude() / 1000000.0;
  gps.longitude = (double)nmea.getLongitude() / 1000000.0;

  // Get altitude (NMEA returns millimeters)
  long alt = 0;
  if (nmea.getAltitude(alt)) {
    gps.altitude = (float)alt / 1000.0;  // Convert to meters
  } else {
    gps.altitude = 0.0;
  }

  // Get HDOP (accuracy indicator, NMEA returns tenths)
  gps.hdop = (float)nmea.getHDOP() / 10.0;
  ...
  if (nmea.isValid()) {
    ...
    gps.valid = (nmea.getHDOP() <= 250);  // HDOP <= 25.0 considered valid
  } else {
    strcpy(gps.timestamp, "NO_GPS_FIX");
    gps.valid = false;
  }
